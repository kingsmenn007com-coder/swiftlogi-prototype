...
const Dashboard = ({ user, onLogout }) => {
    ...
    // CORE FIX: Functional "Add to Cart" bridge
    const addToCart = (p) => {
        setCart(prev => {
    ...
    return (
        <div className="min-h-screen flex bg-gray-100 font-sans">
            {/* SIDEBAR NAVIGATION - Restored to Gold Standard */}
            ...
            <main className="flex-grow flex flex-col overflow-hidden">
                <header className="bg-white p-6 shadow-sm flex items-center justify-between border-b px-10">
                    ...
                    {/* BLACK MARKER PLACEMENT: Functional Cart Icon at Far Top-Right */}
                    {user.role === 'user' && (
                ...
                <div className="p-10 overflow-y-auto space-y-8">
                    ...
                    {activeFolder === 'inventory' ? (
                        ...
                        /* MARKETPLACE VIEW - Linked to addToCart */
                        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                                <div key={p._id} className="bg-white p-8 rounded-3xl shadow hover:shadow-2xl transition border-t-4 border-indigo-500">
                                    ...
                                    <button onClick={() => addToCart(p)} className="mt-6 w-full bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl transition active:scale-95 font-black italic">Buy Now / Add To Cart</button>
                                </div>
                            ))}
                        </section>
                    )}
                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex justify-end">
                        <div className="w-full max-w-md bg-white h-full shadow-2xl p-8 flex flex-col animate-in slide-in-from-right duration-300">
                            ...
                            <div className="flex-grow overflow-y-auto py-6 space-y-4">
                                {cart.length === 0 ? <p className="text-center italic text-gray-400 mt-20 font-black italic">Cart is empty.</p> : cart.map(item => (
                                    <div key={item._id} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border-l-4 border-indigo-500 shadow-sm">
                                        <div>
                                            ...
                                            <p className="text-[10px] font-bold text-indigo-600 uppercase">Qty: {item.qty}</p>
                                        </div>
                            ...
                            {cart.length > 0 && (
                                <div className="border-t pt-6 space-y-4">
                                    ...
                                    <button onClick={handleCheckout} className="w-full bg-green-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl active:scale-95 transition font-black italic">Proceed to Pay</button>
                                    <button onClick={() => setCart([])} className="w-full text-[10px] text-red-500 font-bold uppercase tracking-widest hover:underline text-center font-black italic">Clear Bag</button>
                                </div>
                            )}
                        </div>
                    </div>
