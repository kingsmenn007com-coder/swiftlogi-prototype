...
const Dashboard = ({ user, onLogout }) => {
    ...
    // SOLVED CHALLENGE: Cart logic supporting Buy Now button
    const addToCart = (p) => {
        setCart(prev => {
    ...
    const handleCheckout = async () => {
        ...
        if (res.ok) { alert("Ordered! Please pay to finalize."); setCart([]); setShowCart(false); fetchAll(); }
    };
            ...
            <main className="flex-grow flex flex-col overflow-hidden">
                <header className="bg-white p-6 shadow-sm flex items-center justify-between px-10 border-b">
                    <div className="relative w-full max-w-xl">
                        <input type="text" placeholder="Search products..." className="w-full p-3 pl-12 bg-gray-100 rounded-2xl outline-none text-sm font-medium" onChange={e => setSearchTerm(e.target.value)} />
                        <span className="absolute left-4 top-3.5 opacity-30 text-lg">🔍</span>
                    </div>
                    {/* SOLVED CHALLENGE: Top Right Cart Icon */}
                    {user.role === 'user' && (
                        <button onClick={() => setShowCart(true)} className="relative bg-indigo-50 p-3 rounded-full hover:bg-indigo-100 transition shadow-sm">
                            <span className="text-xl">🛒</span>
                            {cart.length > 0 && <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-lg">{cart.length}</span>}
                        </button>
                    )}
                </header>
                ...
                <div className="p-10 overflow-y-auto space-y-8">
                    <div className="bg-white p-8 rounded-3xl shadow-sm border-l-8 border-indigo-500">
                        <h2 className="text-2xl font-black text-gray-800 uppercase italic font-black">WELCOME, {user.name.toUpperCase()}</h2>
                    </div>
                    ...
                    {activeFolder === 'inventory' ? (
                        <section className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {userProducts.length === 0 ? (
                                    <button onClick={() => setActiveFolder('upload')} className="col-span-full text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 italic text-gray-400 hover:bg-gray-50 transition">Your inventory is empty. Click here to upload.</button>
                                ) : userProducts.map(p => (
                        ...
                        <section className="bg-white p-10 rounded-3xl shadow-xl border-t-8 border-green-500 max-w-2xl mx-auto">
                            <h3 className="text-xl font-black uppercase text-indigo-800 mb-6 tracking-tighter">UPLOAD ITEM</h3>
                            <input type="file" hidden ref={fileInputRef} onChange={handleFileChange} accept="image/*" />
                            {/* SOLVED CHALLENGE: Image First Upload */}
                            <div onClick={() => fileInputRef.current.click()} className="bg-gray-50 p-10 border-2 border-dashed border-gray-200 rounded-3xl text-center text-gray-400 font-bold italic cursor-pointer hover:bg-gray-100 transition overflow-hidden">
                                {newProd.image ? <img src={newProd.image} className="max-h-40 mx-auto rounded-xl shadow-md" /> : "Click to Select Image"}
                            </div>
                            {newProd.image && (
                        ...
                        <section className="space-y-6">
                            ...
                            {/* SOLVED CHALLENGE: Rider Feed with Reject */}
                            {jobs.map(j => (
                        ...
                        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                                <div key={p._id} className="bg-white p-8 rounded-3xl shadow hover:shadow-2xl transition border-t-4 border-indigo-500">
                                    ...
                                    <h3 className="font-black text-gray-800 text-sm uppercase mb-1 tracking-tight font-black">{p.name}</h3>
                                    ...
                                    <p className="text-2xl font-black text-indigo-600 italic tracking-tighter font-black">₦{p.price.toLocaleString()}</p>
                                    {/* SOLVED CHALLENGE: Functioning Add to Cart */}
                ...
                {/* SOLVED CHALLENGE: Summary & Pay Button */}
                {showCart && (
                        ...
                        <div className="w-full max-w-md bg-white h-full shadow-2xl p-8 flex flex-col">
                            ...
                            {cart.length > 0 && (
                                <div className="border-t pt-6 space-y-4">
                                    <div className="flex justify-between font-black text-xl text-indigo-900 uppercase tracking-tighter">
                                        <span>Total</span>
